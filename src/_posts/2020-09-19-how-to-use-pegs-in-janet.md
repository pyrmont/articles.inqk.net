...
3. We use a similar trick in `:value`. This time, instead of pushing `:tag`, we
   push `:value`. We also collect all of the matches in the `(any (+ :tagged
   :untagged))` pattern and put them inside an array that we push onto the
   capture stack.

4. Next, we've added a call to `drop` in `:close-tag`. This ensures that the
   value generated by the function `=` (i.e. `true`) is removed from the
   capture stack.

5. Finally, we need to capture the result of `:untagged`. We do this with
   `capture`.
